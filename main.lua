...
--     if next(SMODS.find_card('j_drx1_collector')) and (self.ability.set == 'Planet' or (self.ability.set == 'Booster' and self.ability.name:find('Celestial'))) then
        ...
        if context.cardarea == G.jokers then
            ...
            if card.ability.extra.flag == 1 and context.drawing_cards and not next(SMODS.find_card('j_drx1_abramar')) then
                ...
                if context.amount >= 0 then
                    ...
                    return {
                        cards_to_draw = card.ability.extra.draw
                    }
                end
            end
        ...
        G.E_MANAGER:add_event(Event({
            ...
            func = function()
                if #G.jokers.cards + G.GAME.joker_buffer < G.jokers.config.card_limit then
                    card_eval_status_text(card, 'extra', nil, nil, nil, {message = 'Warning!', colour = G.C.MULT})
                    local card = create_card('Joker', G.jokers, nil, 0, nil, nil, 'j_marble')
                    card:add_to_deck()
                    G.jokers:emplace(card)
                    card:start_materialize()
                    G.GAME.joker_buffer = 0
...
SMODS.Joker {
    ...
    calculate = function(self, card, context)
        ...
        if context.remove_playing_cards then
            ...
            if destroyed > 0 then
                card.ability.extra_value = card.ability.extra_value + (destroyed*card.ability.extra.money)
                card:set_cost()
                return {
...
SMODS.Joker {
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {localize { type = 'name_text', set = 'Tarot', key = card.ability.extra.tarot },
            colours = {
                    G.C.SECONDARY_SET.Tarot,
                    {1, 1, 1, 1}
            }
    ...
    calculate = function(self, card, context)
        ...
        if context.using_consumeable and context.consumeable.config.center.key == card.ability.extra.tarot then
...
--Consumptive Reflex (Add can sound)
SMODS.Joker {
    key = 'reflex',
    atlas = 'Jokers',
    pos = {
        ...
        if context.after and SMODS.calculate_round_score() + G.GAME.chips < G.GAME.blind.chips and #G.consumeables.cards >= 1 and not context.blueprint and G.GAME.current_round.hands_left == 0 then
            ...
            for i = 1, #G.consumeables.cards do
                if not G.consumeables.cards[i].getting_sliced then
                    bricks[#bricks + 1] = G.consumeables.cards[i]
                end
            end
            local destroyed_brick = pseudorandom_element(bricks, 'drx1_reflex')

            if destroyed_brick then
                destroyed_brick.getting_sliced = true
                G.E_MANAGER:add_event(Event({
                    func = function()
                        card:juice_up(0.8, 0.8)
                        destroyed_brick:start_dissolve({ G.C.DARK_EDITION }, nil, 1.6)
                        return true
                    end
                }))
                ease_hands_played(1)
...
SMODS.Joker {
    ...
    calculate = function(self, card, context)
        if not context.blueprint and context.end_of_round and context.main_eval and G.GAME.blind_on_deck == "Small" then
            if pseudorandom('billion') < G.GAME.probabilities.normal / card.ability.extra.odds then
                card.ability.extra.odds = card.ability.extra.odds + 1
                ease_ante(-1)
                G.GAME.round_resets.blind_ante = G.GAME.round_resets.blind_ante or G.GAME.round_resets.ante
                G.GAME.round_resets.blind_ante = G.GAME.round_resets.blind_ante - 1
                return{
                    message = "eye of the universe",
                    colour = G.C.DARK_EDITION,
                }
            end
        end
